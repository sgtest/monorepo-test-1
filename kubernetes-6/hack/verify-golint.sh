...
export IFS=$'\n'
# NOTE: when "go list -e ./..." is run within GOPATH, it turns the github.com/sourcegraph/monorepo-test-1/kubernetes-6
...
all_packages=(
	$(go list -e ./... | egrep -v "/(third_party|vendor|staging/src/k8s.io/client-go/pkg|generated|clientset_generated)" | sed -e 's|^github.com/sourcegraph/monorepo-test-1/kubernetes-6/||' -e "s|^_${KUBE_ROOT}/\?||")
)
linted_packages=(
	$(cat $linted_file)
)
